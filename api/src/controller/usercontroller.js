...
const hqdata = require('../models/HQ')
...
exports.get_local_bank_transfer = async(req,res,next) => {
    Local_Bank_Data.find({ "status" : "Submitted"})
    .then(data => {
        //console.log("data in usercontroller : " , data)
        res.render('dashboard.ejs',{data:data})
   })
}
exports.local_bank_transfer = async (req,res,next) => {
    let data = await Local_Bank_Data.find({ "status" : "Submitted"})
    console.log("response from local bank db : ",data)
      
           
    
    
    let transactionID = data[0].transactionID;
    let amountinr = data[0].amount;
        amount = amountinr/currencyexchg;
    let time = Date.now();
    let branchcode = data[0].branchcode;
    let GRICcode = 'IN12345';
    ...
    .then(responsefromdb => {
        console.log("Data added to  international db : ", responsefromdb)
        res.send("Transaction has been initiated");
    })
}
exports.get_international_bank_transfers = async (req,res,next) => {
    ...
    .then(data => {
        console.log("data : " , data)
        data[0].amount = +data[0].amount.toFixed(2);
        res.render('dashboardint.ejs',{data:data})
    })
}
...
exports.hqgetdata = (req,res,next) => {
    hqdata.find()
    .then(data => {
        console.log("Balance of local branch : ",data)
        data[0].amount = +data[0].amount.toFixed(2);
        res.render('dashboardHQ.ejs',{data:data})
    })
}
...
exports.get_local_branch_fund = (req,res,next) => {
    ...
    .then(data => {
        console.log("Balance of local branch : ",data)
        data[0].balance = +data[0].balance.toFixed(2);
        res.render('branchbal.ejs',{data:data})
    })
}
...
exports.get_int_branch_fund = (req,res,next) => {
    ...
    .then(data => {
        ...
        data[0].balance = +data[0].balance.toFixed(2);
        res.render('hqbal.ejs',{data: data});
    })
}