...
const gettinglast = async (req, res, next) => {
    // let key = req.params.key;
    let address = req.body.address
    console.log("Address in app : ",address)
    const context = createContext('secp256k1')
    let privateKey = context.newRandomPrivateKey().asHex()
    // console.log("key ->", key, typeof (key));

    let ehr = new client(privateKey);
    let data = await ehr.getTransferListing(address);
    console.log("transfer data : ",data)
    let returndata = {
        message : "encrypted text received",
        data: data
...
app.post('/get_data_from_state',gettinglast)
